...
public class NearbySearchServiceImpl implements NearbySearchService{
	...
	public Places search(NearbySearchRequest nearbySearchRequest) throws ServiceException{
		Places places= null;
		try{
			Category category = categoryDAO.getCategoryById(nearbySearchRequest.getCategoryId());
			if(category==null){
				throw new ServiceException(ServiceErrorCodes.ERR_001,"Category does not exist");
			}

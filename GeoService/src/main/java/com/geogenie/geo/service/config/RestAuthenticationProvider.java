...
public class RestAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		    ...
		    if(userType.equals(Constants.DEVICE_PREFIX)){
		    	...
		    	try{
		    		SmartDevice device = this.userService.getSmartDeviceDetails(deviceId);
		    		deviceKey = device.getPrivateKey();
		    	}catch(ServiceException exception){
		    		throw new BadCredentialsException(exception.getMessage());
		    	}
		        
		        String signature = calculateSignature(deviceKey,restToken.getTimestamp());
		        ...
		        restToken = new RestToken(secret, credentials, restToken.getTimestamp(), authorities);
		    	...
		    	try{
		    		...
		    		if(socialDetails == null || socialDetails.isEmpty()){
		    			...
		    			throw new ServiceException(ServiceErrorCodes.ERR_001, "Invalid username or password.");
		    		}
	    			for(UserSocialDetail socialDetail : socialDetails){
	    				if(socialDetail.getSocialDetailType().equals(SocialDetailType.USER_EXTERNAL_ID)){
	    			...
	    			String signature = calculateSignature(externalId, restToken.getTimestamp());
	    			 if (!credentials.getSignature().equals(signature)) {
	 		            throw new BadCredentialsException("Invalid username or password.");
	 		        }
	...
	private List<GrantedAuthority> convertRoleToGA(List<Role> userRoles){
		...
		for(Role role: userRoles){
			GrantedAuthority grantedAuthority = new SimpleGrantedAuthority(role.getUserRoleType());
